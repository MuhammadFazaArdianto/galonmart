<div class="hidden lg:block">
    ...
    <div class="pb-6 border-b border-gray-200">
        ...
        <div x-cloak x-show="categoryMenu" class="pt-6" id="filter-section-1">
            <div class="space-y-4">
                @foreach ($categories as $category)
                    {{-- @if (!$category->parent_id) --}}
                    <div class="flex items-center">
                        <input id="filter-category-0" name="category[]" value="{{ $category->id }}" type="checkbox"
                            wire:model="f_categories"
                            class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        <label for="filter-category-0" class=" rtl:mr-3 ltr:ml-3 text-sm text-gray-600">
    ...
    <div class="py-6 border-b border-gray-200">
        ...
        <!-- Filter section, show/hide based on section state. -->
        <div x-show="brandMenu" style="display:none" class="pt-6" id="filter-section-2">
            <div class="space-y-4">
                @foreach ($brands as $brand)
                    <div class="flex items-center">
                        <input id="filter-brand-{{ $brand->id }}" name="brand[]" value="{{ $brand->id }}"
                            type="checkbox" wire:model="f_brands"
                            class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        <label for="filter-brand-{{ $brand->id }}" class=" rtl:mr-3 ltr:ml-3 text-sm text-gray-600">
