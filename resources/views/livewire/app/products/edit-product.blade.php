<div class="py-10 bg-gray-100 screen">
    <h2 class="sr-only">Edit Product</h2>
    <div class="flex justify-between pb-2 border-b">
        <h3 class="text-lg font-medium text-gray-900">{{ __('Edit Product') }}</h3>
    ...
    <div class="grid grid-cols-1 gap-2 divide-x-2 lg:grid-cols-3 rtl:divide-x-reverse">
        <!-- Products info form -->
        <form id="new_product_form" class="relative lg:col-span-2" wire:submit.prevent="update">
            <div class="mt-6">
                <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                    ...
                    {{-- {{ dd($excerpt) }} --}}
                    <div x-cloak class="sm:col-span-2" x-data="{
                        content: @entangle('excerpt'),
                        ...
                        <div>
                            <div class="relative mt-1 rounded-md shadow-sm">
                                <textarea  rows="3" wire:model.defer="excerpt"
                    ...
                    <div class="sm:col-span-2">
                        <div wire:ignore>
                            ...
                            <div class="mt-1">
                                <textarea x-data x-ref='ckEditor2' x-init="ClassicEditor
                                    .create($refs.ckEditor2, {
            ...
            <div class="pt-2 mt-4 border-t sm:flex sm:justify-end">
        ...
        <div>
            ...
            <form wire:submit.prevent="updateFeatures"
                class="flex flex-col w-full p-2 mt-6 overflow-hidden bg-white rounded-md shadow-md ltr:ml-2 rtl:mr-2 hover:shadow-lg">
                ...
                <fieldset class="py-2 space-y-5">
                    ...
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                            <input id="new" aria-describedby="new-description" name="new" type="checkbox" wire:model.defer="new"
                                class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        </div>
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                            <label for="new" class="font-medium text-gray-700">{{ __('New Product') }}</label>
                        </div>
                    </div>
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                ...
                <div class="px-2 pt-2 mt-4 mb-2 border-t sm:flex sm:justify-end">
                    {{-- <div class="flex justify-end flex-1 w-0 pt-2 ltr:ml-3 rtl:mr-3">
                        <p class="text-sm font-medium text-green-600">{{ __('Successfully saved!') }}</p>
                    </div> --}}
            ...
            <form  wire:submit.prevent="updateDisplayOptions"
                class="flex flex-col w-full p-2 mt-4 overflow-hidden bg-white rounded-md shadow-md ltr:ml-2 rtl:mr-2 hover:shadow-lg">
                ...
                <fieldset class="py-2 space-y-5">
                    ...
                    <div class="relative flex items-start">
                        ...
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                            <label for="hide-price" class="font-medium text-gray-700">{{ __('Hide Price') }}</label>
                        </div>
                    </div>
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                            <input id="hide-options" aria-describedby="hide-options-description" name="hide-options" wire:model.defer="hide_options"
                                type="checkbox"
                                class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        </div>
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                    ...
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                            <input id="hide-excerpt" aria-describedby="hide-excerpt-description" name="hide-excerpt" wire:model.defer="hide_excerpt"
                                type="checkbox"
                                class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        </div>
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                    ...
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                            <input id="hide-preview" aria-describedby="hide-preview-description" name="hide-reviews" wire:model.defer="hide_reviews"
                                type="checkbox"
                                class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        </div>
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                    ...
                    <div class="relative flex items-start">
                        ...
                        <div class="text-sm ltr:ml-3 rtl:mr-3">
                            <label for="show-stock" class="font-medium text-gray-700">{{ __('Show Stock') }}</label>
                        </div>
                    </div>
                </fieldset>
                <div class="px-2 pt-2 mt-4 mb-2 border-t sm:flex sm:justify-end">
            ...
            <form wire:submit.prevent="updateImages"
                class="w-full p-2 mt-6 overflow-hidden bg-white rounded-md shadow-md ltr:ml-2 rtl:mr-2 hover:shadow-lg">
                <div class="pb-1 mb-4 border-b">
                ...
                <div>
                    <div class="flex flex-col" x-data="drop_file_component()">
                        <div x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true"
                            ...
                            <label x-on:drop="dropingFile = false" x-on:drop.prevent="handleFileDrop($event)"
                                ...
                                <div class="flex items-center justify-center flex-1 px-4 py-2 mt-10">
                                    ...
                                    <span
                                        class="text-sm text-gray-600 ltr:ml-2 rtl:mr-2">{{ __('Browse images') }}
                                </div>
                        ...
                        @if (count($images) || count($old_images))
                            <div class="grid grid-cols-1 p-2 my-10 border border-blue-400 rounded-md lg:grid-cols-3">
                                @foreach ($old_images as $old_image)
                                    <div wire:key="{{ $loop->index }}"
                                        ...
                                        <button
                                            ...
                                            type="button" wire:click="removeOldPreview({{ $loop->index }})">
                                        ...
                                        <div>
                                            <img class="absolute inset-0 z-0 object-fill w-full h-full border-2 border-white"
                                                src="{{ asset('storage/' . $old_image->path) }}" />
                                        </div>

                                        <div
                                            class="absolute bottom-0 left-0 right-0 flex flex-col p-2 text-xs bg-white bg-opacity-50">
                                            <span
                                                class="w-full font-bold text-gray-900 truncate">{{ $old_image->getTranslation('name', App::getLocale()) }}
                                            </span>
                                            <span
                                                class="text-xs text-gray-900">{{ Helper::bytesToHuman($old_image->size) }}
                                            </span>
                                        </div>
                                    </div>
                                @endforeach
                                @foreach ($images as $image)
                                    <div wire:key="image_{{ $loop->index }}"
                                        class="relative flex flex-col items-center overflow-hidden text-center bg-gray-100 border rounded cursor-move select-none pt-[100%]">
                                        <button
                                            class="absolute top-0 right-0 z-50 p-1 bg-white rounded-bl focus:outline-none"
                                            type="button" wire:click="removeNewPreview({{ $loop->index }})">
                                            <svg class="w-4 h-4 text-red-700" xmlns="http://www.w3.org/2000/svg"
                                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
