<div class="screen py-10 bg-gray-100 min-h-screen" x-data="{ openModel: false }">
    ...
    <div class="w-full sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-xl font-semibold text-gray-900">{{ __('All Brands') }}</h1>
        </div>
                            ...
                            @if (count($brands))
                                @foreach ($this->brands as $brand)
                                    <tr class="text-xs text-gray-700">
                                        <td class="px-4 py-3 whitespace-nowrap">{{ $brand->sort }}</td>
                                        <td class="px-2 py-3 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div
                                                    ...
                                                    @if ($brand->media())
                                                        <img class="object-cover w-full h-full rounded-sm"
                                                            src="{{ asset('storage/' . $brand->media()->path) }}"
                                                            alt="" loading="lazy">
                                                    @else
                                                ...
                                                <div>
                                                    <a
                                                        href="{{ route('brand', $brand->getTranslation('slug', App::getLocale())) }}">{{ $brand->getTranslation('name', App::getLocale()) }}</a>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <a
                                                href="{{ route('brand', $brand->getTranslation('slug', App::getLocale())) }}">{{ $brand->getTranslation('slug', App::getLocale()) }}</a>
                                        </td>
                                        <td class="px-2 py-3 whitespace-nowrap">{{ count($brand->products) }}
                                        </td>
                                        <td class="px-2 py-3 whitespace-nowrap">
                                            <div class="relative">
                                                <button wire:click="enabled({{ $brand->id }})" type="button"
                                                    class="{{ $brand->enabled ? 'bg-indigo-600' : 'bg-gray-200' }} relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                    role="switch" aria-checked="false">
                                                    <span aria-hidden="true"
                                                        class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200
                                                        {{ $brand->enabled ? 'ltr:translate-x-5 rtl:-translate-x-5' : '' }}"></span>
                                                </button>
                                            </div>
                                        </td>
                                        <td class="px-2 py-3 whitespace-nowrap">
                                            <div class="flex items-center space-x-4 rtl:space-x-reverse">
                                                <button wire:click="view({{ $brand->id }})"
                ...
                <div class="flex items-end justify-center min-h-full p-4 text-center sm:items-center sm:p-0">
                    <div
                        ...
                        <div class="flex items-center justify-center w-full h-12 px-2 mx-auto text-white bg-blue-500">
                            ...
                            <h1>{{ !$this->readOnly ? __('Add Brand') : __('View Brand') }}
                            </h1>
                        </div>
                        <button
                        ...
                        <div class="px-4 pt-5 pb-4 ltr:text-left rtl:text-right sm:p-6">
                            <div>
                                ...
                                <div class="mt-4">
                                    <label for="name_id"
                                        class="block text-sm font-medium text-gray-700">{{ __('Name') }}&nbsp;{{ __('Name Indonesia') }}</label>
                                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div
                                ...
                                <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">
                                    {{ __('Delete brand') }}
                                    <span
                                        class="font-semibold">{{ $this->brand->getTranslation('name', App::getLocale()) }}</span>
                                </h3>
                                <div class="mt-2">
